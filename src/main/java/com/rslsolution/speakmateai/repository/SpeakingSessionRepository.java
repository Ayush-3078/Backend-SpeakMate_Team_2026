...
public interface SpeakingSessionRepository extends JpaRepository<SpeakingSession, Long> {
	...
	@Query("SELECT s FROM SpeakingSession s WHERE s.user.id = :userId AND s.overallScore IS NOT NULL ORDER BY s.createdAt DESC")
	List<SpeakingSession> findByUserIdAndOverallScoreIsNotNullOrderByCreatedAtDesc(@Param("userId") Long userId);
}
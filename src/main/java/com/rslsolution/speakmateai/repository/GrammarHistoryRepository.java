...
public interface GrammarHistoryRepository extends JpaRepository<GrammarHistory, Long> {
	...
	@Query("SELECT AVG(g.grammarScore) FROM GrammarHistory g WHERE g.user.id = :userId AND g.grammarScore IS NOT NULL")
	Double findAverageGrammarScoreByUserId(@Param("userId") Long userId);

	@Query("SELECT COUNT(g) FROM GrammarHistory g WHERE g.user.id = :userId AND g.createdAt BETWEEN :start AND :end")
	...
	@Query("SELECT g FROM GrammarHistory g WHERE g.user.id = :userId AND g.createdAt BETWEEN :start AND :end ORDER BY g.createdAt DESC")

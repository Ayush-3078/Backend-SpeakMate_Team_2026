...
public class SchoolDashboardServiceImpl implements SchoolDashboardService {
	...
	public SchoolDashboardResponse getSchoolDashboard() {
		...
		return SchoolDashboardResponse.builder().totalStudents((long) students.size()).activeStudents(activeStudents)
				.inactiveStudents(inactiveStudents).totalTeachers(totalTeachers).totalClasses(totalClasses)
				.totalResults(totalResults).averageResultPercentage(Math.round(avgPercentage * 100.0) / 100.0)
				.excellentResults(excellent).goodResults(good).passResults(passCount).failResults(fail)
				.totalLessonsCompleted(totalLessonsCompleted).build();
	}
}

...
public class AdminAuthServiceImpl implements AdminAuthService {
	...
	public void forgotPassword(ForgotPasswordRequest request) {
		...
		sendAsyncEmail(admin.getEmail(), "Your SpeakMateAI Admin Password Reset OTP", htmlContent, otp);
	}
		...
		java.util.concurrent.CompletableFuture.runAsync(() -> {
			...
			try {
				if (mailSender != null) {
					...
					helper.setFrom("dnyaneshwaralgule2003@gmail.com", "SpeakMateAI Admin");
					helper.setTo(toEmail);
					helper.setSubject(subject);
					helper.setText(htmlContent, true);
					mailSender.send(message);
					System.out.println("[SMTP Admin Email Sent] Successfully sent to: " + toEmail);
				} else {
					System.out.println("[SMTP Offline] Admin OTP for " + toEmail + " is: " + otp);
				}
			} catch (Exception ex) {
				System.err.println("[SMTP Error] Failed to send email to " + toEmail + ": " + ex.getMessage());
				System.out.println("[Fallback Log] Admin OTP for " + toEmail + " is: " + otp);
			}
		});
	}
}
